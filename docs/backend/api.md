...
| `/admin/durable/provider-outcomes` | GET | provider fetch outcomes filtered by symbol/status |
| `/admin/durable/operational-runs` | GET | durable job/recovery/provider/admin run records filtered by type/status |
| `/admin/durable/ingestion-runs` | GET | durable provider/flat-file/recovery batch ingestion records filtered by source/status |
...
| `/admin/hot-cache/rebuild` | POST | rebuild latest-week Redis cache from durable `bars_1m`; defaults to `dryRun=true` |
...
- `provider_no_data` only when the latest provider outcome for the symbol is
  explicitly `empty`.
...
- `backfill_pending` when no durable bars or provider-empty evidence exists yet.
...
| `/admin/durable/bars/latest` | `symbols` optional comma-separated list, `source=live_ws\|provider_rest\|flat_file\|recovery` optional, `limit` optional capped at 500 |
...
| `/admin/durable/ingestion-runs` | `source=provider_rest\|flat_file\|recovery` optional, `status=started\|success\|failed` optional, `limit` optional capped at 500 |
...
curl -H "X-API-Key: $HUB_API_KEY" "http://localhost:3001/admin/durable/provider-outcomes?symbol=ESH6&status=empty" | jq
curl -H "X-API-Key: $HUB_API_KEY" "http://localhost:3001/admin/durable/ingestion-runs?source=provider_rest&status=success" | jq
